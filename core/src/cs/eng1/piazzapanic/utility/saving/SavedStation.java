    ...
    public SavedStation(Station station) {
        ...
        if (station instanceof RecipeStation) {
            ...
            if (station instanceof ChoppingStation) {
                ...
                items[0] = choppingStation.currentIngredient != null ? new SavedFood(choppingStation.currentIngredient)
                        : null;
                type = "Chopping";
            } else if (station instanceof CookingStation) {
                CookingStation cookingStation = (CookingStation) station;
                items[0] = cookingStation.currentIngredient != null ? new SavedFood(cookingStation.currentIngredient)
                        : null;
                type = "Cooking";
            } else if (station instanceof GrillingStation) {
                GrillingStation grillingStation = (GrillingStation) station;
                items[0] = grillingStation.currentIngredient != null ? new SavedFood(grillingStation.currentIngredient)
                        : null;
                type = "Grilling";
            } else {
