...
public class IngredientTests {
    ...
    public void testFromString() {
        ...
        assertEquals("The class of the ingredient found should be the correct class.", Patty.class, ingredient.getClass());
    }
    /*
    ...
    public void testIsCooked() {
        Ingredient ingredient = Ingredient.fromString("patty", textureManager);
        assertFalse("The ingredient shouldn't be immediately cooked.", ingredient.getCooked());

        ingredient.setIsCooked(true);
        assertTrue("getCooked should be true when cooked is true.", ingredient.getCooked());

        ingredient.setIsCooked(false);
        assertFalse("getCooked should be false when cooked is false.", ingredient.getCooked());
    }
    /*
    ...
    public void testIsChopped() {
        ...
        assertFalse("An ingredient should not be chopped immediately.", ingredient.getChopped());

        ingredient.setChopped(true);
        assertTrue("getChopped should be true when chopped is true.", ingredient.getChopped());

        ingredient.setChopped(false);
        assertFalse("getChopped should be false when chopped is false.", ingredient.getChopped());
    }
    /*
    ...
    public void testIsGrilled() {
        Ingredient ingredient = Ingredient.fromString("patty", textureManager);
        assertFalse("An ingredient should not be grilled immediately.", ingredient.getGrilled());

        ingredient.setIsGrilled(true);
        assertTrue("getGrilled should be true when grilled is true.", ingredient.getGrilled());

        ingredient.setIsGrilled(false);
        assertFalse("getGrilled should be false when grilled is false.", ingredient.getGrilled());
    }
    /*
    ...
    public void testToString() {
        ...
        Ingredient lettuce = Ingredient.fromString(
            "lettuce",
            textureManager
        );
        ...
        assertEquals("Ingredient should be changed to _choppedwhen fully chopped.", "lettuce_chopped", lettuce.toString());
    ...
    public void testUseable() {
        ...
        assertTrue("Ingredient should be useable by default", ingredient.getUseable());

        ingredient.setUseable(false);
        assertFalse("getUseable should be false when useable is false.", ingredient.getUseable());

        ingredient.setUseable(true);
        assertTrue("getUseable should be true when useable is true.", ingredient.getUseable());
    }
}
