...
public class ChefTests {
    ...
    public void chefStackTests() {
        ...
        // For whatever reason, calling popIngredient breaks the CI, so I'll just do
        // this without the peek() check.
        chef.getStack().pop();
        chef.grabItem(patty);
        assertTrue(chef.hasIngredient());
    ...
    public void movementTests() {
        ...
        move();
        assertEquals(0, chef.getBody().getPosition().x, 1f);
        assertEquals(-4, chef.getBody().getPosition().y, 1f);
        clear();
