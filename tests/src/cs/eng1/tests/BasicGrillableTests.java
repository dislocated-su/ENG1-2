...
public class BasicGrillableTests {
    ...
    public void getGrillResultTests() {
        ...
        assertEquals(
                ...
                patty.getGrillResult().toString());
        patty.grillTick(1);
        assertEquals("A BasicGrillable should be considered grilled even if it continues cooking after being finished.", "patty_grilled", patty.getGrillResult().toString());
        patty.grillTick(2);
        assertEquals("A BasicGrillable should be ruined after being fully cooked and continuing cooking for too long.", "patty_ruined", patty.getGrillResult().toString());
        patty.grillTick(400);
        assertEquals("A BasicGrillable should remain ruined for as long as it keeps being cooked after being ruined.", "patty_ruined", patty.getGrillResult().toString());
    }
    /*
    ...
    public void getTextureTests() {
        final Texture rawPatty = foodTextureManager.getTexture(
            "patty_raw"
        );
    ...
    public void grillStepCompleteTests() {
        assertFalse("A BasicGrillable shouldn't have a complete grill step without being grilled at all.", patty.grillStepComplete());
        patty.grillTick(1);
        assertFalse("A BasicGrillable shouldn't have a complete grill step without being grilled for the full grillStep time.", patty.grillStepComplete());
        patty.grillTick(1);
        assertTrue("A BasicGrillable should have a complete grill step after being grilled for the full grillStep time.", patty.grillStepComplete());
        patty.grillTick(1);
        assertTrue("A BasicGrillable should have a complete grill step after being grilled for more than the full grillStep time.", patty.grillStepComplete());
        patty.flip();
        assertFalse("A BasicGrillable shouldn't have a complete grill step without being grilled at all after flipping.", patty.grillStepComplete());
        patty.grillTick(1);
        assertFalse("A BasicGrillable shouldn't have a complete grill step without being grilled for a grillStep after flipping.", patty.grillStepComplete());
        patty.grillTick(1);
        assertTrue("A BasicGrillable should have a complete grill step after being grilled for the full grillStep time.", patty.grillStepComplete());
        patty.grillTick(1);
        assertTrue("A BasicGrillable should have a complete grill step after being grilled for more than the full grillStep time.", patty.grillStepComplete());
